...
The primary data structure was a built-in Python dictionary, which was tasked with managing contact entries. This structure was chosen due to its efficiency in handling these particular operations (average complexity for add, update, delete, get operations).
...
- `contact_book.py`: the Python script that defines the ContactBook class which encapsulates the core functionalities of the application, it interacts with the user and uses ContactBook class methods at the texting part to check the desired functionality.
...
The `contact_book.py` file serves as an interface between the user and the `ContactBook` class instance. It gets user inputs, calls the necessary methods and handles the application flow.
